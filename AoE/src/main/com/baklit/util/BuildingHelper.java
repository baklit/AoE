	...
	public static void selectBuilding(Building building, Player player){
		...
		for(int x=0;x<=width;){
			for(int z=0;z<=length+1;){
				if(x==0 || x==width){
					Location location = new Location(player.getWorld(), baseLocation.getX()+1+x,baseLocation.getY()+1+height, baseLocation.getZ()+z);
		...
		player.sendMessage("This building is " + building.getPercentageBuilt() + "% built and building at " + building.getBuildSpeed());
		...
		for(int x=0;x<=width;){
			for(int z=0;z<=length+1;){
				if(x==0 || x==width){
					...
					player.sendBlockChange(location, resetBlock.getType(), resetBlock.getData());
				}
				if(z==0 || z==length+1){
					Location location = new Location(player.getWorld(), baseLocation.getX()+1+x,baseLocation.getY()+1+height, baseLocation.getZ()+z);
					Block resetBlock = player.getWorld().getBlockAt(location);
